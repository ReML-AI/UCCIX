...
from peft import LoraConfig, TaskType, get_peft_model, PeftModel, get_peft_model_state_dict, IA3Config
from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR
from custom_trainer import TrainerWithCustomSampler
import pandas as pd

os.environ['WANDB_PROJECT'] = 'irish_llm_pretrain'

class SavePeftModelCallback(transformers.TrainerCallback):
    def save_model(self, args, state, kwargs):
        if state.best_model_checkpoint is not None:
            checkpoint_folder = os.path.join(state.best_model_checkpoint, "pt_peft_model")
        else:
...
class MyTrainingArguments(TrainingArguments):
    trainer: Optional[str] = field(default="custom")
...
def main():
    ...
    with training_args.main_process_first(desc="dataset map tokenization and grouping"):
        ...
        for idx, file in enumerate(files):
            ...
            try:
                ...
                cache_dir = os.path.join(data_args.data_cache_dir, filename+"_test") # NOTE: TESTING HERE
                os.makedirs(cache_dir, exist_ok=True)
                raw_dataset = pd.read_json(data_file, lines=True)
                raw_dataset = datasets.Dataset.from_pandas(raw_dataset[['text']])
                raw_dataset = datasets.DatasetDict({'train': raw_dataset})
        ...
        lm_datasets = lm_datasets.train_test_split(test_size = data_args.validation_split_percentage, shuffle=False)
    ...
    if not (
       (model_vocab_size==32000 and len(tokenizer)==35483) or \
       (model_vocab_size==32000 and len(tokenizer)==52914) or \
       ...
       (model_vocab_size==49954 and len(tokenizer)==49954)
        ...
        raise ValueError(
            ...
            "- Continue Pre-training Irish LLaMA based on original LLaMA: 32000 / 35483 \n"
            "- Continue Pre-training Irish LLaMA based on original LLaMA: 32000 / 52914 \n"
